...
parser = ArgumentParser(
    prog='KULpubScheduleToICS',
    description='Publishes public SAP schedule to ICS calendar',
)

parser.add_argument('source')
parser.add_argument('-d', '--destination', default='./schedule.ics')

args = parser.parse_args()
# URL of the semester-view schedule
URL_SCHEDULE = args.source

# Time range regex (both NL and EN compatible)
REGEXPR_TIME_RANGE = r"(\d+:\d+)\s*\w+\s*(\d+:\d+)"

# Destination ics filename
ICS_FILENAME = args.destination
...
def main():
    ...
    #   4) the dates
    #       * This table has a second row, containing a line seperating
    #         the events
